...
echo 5. check updates robloxzapret
echo 6. Switch Check Updates (%CheckUpdatesStatus%)
echo 7. Switch Game Filter (%GameFilterStatus%)
echo 8. Switch ipset (%IPsetStatus%)
echo 9. update ipset list robloxzapret
echo 10. update hosts file zapret (for discord voice)
echo 11. Run Tests
echo 0. Exit
...
sc query "robloxzapret" >nul 2>&1
if !errorlevel!==0 (
    for /f "tokens=2*" %%A in ('reg query "HKLM\System\CurrentControlSet\Services\robloxzapret" /v robloxzapret 2^>nul') do echo Service strategy installed from "%%B"
)

call :test_service robloxzapret
...
sc create %SRVCNAME% binPath= "\"%BIN_PATH%winws.exe\" !ARGS!" DisplayName= "robloxzapret" start= auto
sc description %SRVCNAME% "robloxzapret DPI bypass software"
sc start %SRVCNAME%
for %%F in ("!file%choice%!") do (
    set "filename=%%~nF"
)
reg add "HKLM\System\CurrentControlSet\Services\robloxzapret" /v robloxzapret /t REG_SZ /d "!filename!" /f
...
set "GITHUB_VERSION_URL=https://raw.githubusercontent.com/cmdwindows/robloxzapret/main/.service/actualversion.txt"
set "GITHUB_RELEASE_URL=https://github.com/cmdwindows/robloxzapret/releases/tag/"
set "GITHUB_DOWNLOAD_URL=https://github.com/cmdwindows/robloxzapret/releases/latest/download/robloxzapret-"
...
if not defined GITHUB_VERSION (
    echo Warning: failed to fetch the latest version. This warning does not affect the operation of robloxzapret
    timeout /T 9
    if "%1"=="soft" exit 
    goto menu
)
...
if !errorlevel!==0 (
    ...
    call :PrintRed "https://github.com/Flowseal/robloxzapret/issues/417"
) else (
...
if !errorlevel!==0 (
    call :PrintRed "[X] Killer services found. Killer conflicts with robloxzapret"
    call :PrintRed "https://github.com/Flowseal/robloxzapret/issues/2512#issuecomment-2821119513"
) else (
...
if !errorlevel!==0 (
    call :PrintRed "[X] Intel Connectivity Network Service found. It conflicts with robloxzapret"
...
if !checkpointFound!==1 (
    call :PrintRed "[X] Check Point services found. Check Point conflicts with robloxzapret"
...
if !errorlevel!==0 (
    call :PrintRed "[X] SmartByte services found. SmartByte conflicts with robloxzapret"
...
if !errorlevel!==0 (
    ...
    call :PrintYellow "[?] VPN services found:!VPN_SERVICES!. Some VPNs can conflict with robloxzapret"
...
if not exist "%gameFlagFile%" (
    ...
    call :PrintYellow "Restart the robloxzapret to apply the changes"
) else (
...
set "hostsUrl=https://raw.githubusercontent.com/Flowseal/zapret/refs/heads/main/.service/hosts"
set "tempFile=%TEMP%\robloxzapret_hosts.txt"
set "needsUpdate=0"
...
echo.
start "" powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0utils\test zapret (update by robloxzapret).ps1"
pause
goto menu
...
if %errorLevel% neq 0 (
    ...
    echo Fix your PATH variable with instructions here https://github.com/Flowseal/zapret/issues/7490
    pause
    exit /b 1
)
exit /b 0
...
if "%extracted%"=="0" (
    echo robloxzapret must be extracted from archive first or bin folder not found for some reason
    pause
    exit
)
exit /b 0
